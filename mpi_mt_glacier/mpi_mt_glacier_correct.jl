...
function forward_problem(V_local::AbstractArray , xx::AbstractArray, nx::Int, dx::Float64, xend::Float64, dt::Float64, tend::Float64, xarr::Vector{Float64}, i1::Int, i2::Int)
	...
	for t in 1:nt
		...
		D .= C .* ((h_capital[1:nx_local] .+ h_capital[2:nx_local+1]) ./ 2.0).^(n+2) .* ((h[2:nx_local+1] .- h[1:nx_local]) ./ dx).^(n-1)
		phi .= -D .* (h[2:nx_local+1] .- h[1:nx_local]) ./ dx
		h[2:nx_local] .= h[2:nx_local] .+ M[2:nx_local] .* dt .- dt/dx .* (phi[2:nx_local] .- phi[1:nx_local-1])

		h[2:nx_local] .= update_h.(h[2:nx_local], b[2:nx_local])
		h_capital[2:nx_local] .= h[2:nx_local] .- b[2:nx_local]
		...
		if size > 1
			...
			if rank == size-1
				...
				h[1] = h[1] + M[1] * dt - dt/dx * (phi[1] - recv_mesg_left[1])
				h[1] = update_h(h[1], b[1])
				h_capital[1] = h[1] - b[1]
			...
			elseif rank == 0
				...
				h[nx_local+1] = h[nx_local+1] + M[nx_local+1] * dt - dt/dx * (recv_mesg_right[1] - phi[nx_local])
				h[nx_local+1] = update_h(h[nx_local+1], b[nx_local+1])
